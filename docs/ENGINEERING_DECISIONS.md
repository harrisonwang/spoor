...
结构化错误：`code: "image_only_pdf"`、中文 reason/hint（明确提示需要 OCR）、
`recoverable: true`。受密码保护的 PDF 同理返回 `code: "encrypted_pdf"`、
`recoverable: false`。结构化错误信封为 `{is_error, code, reason, hint, recoverable}`，
消费者按 `code` 分支；`reason`/`hint` 是中文展示文本，不构成 contract。
全部稳定 code 见 README「结构化错误与 code」一节。
...
- `pith chunk` / 文档分块——文档是顺序读介质，截断是尾部小概率问题：缩小输入或 `--max-output-bytes` 重跑即可。为它造"清单 + 切片 + 收敛"整套机制，是把表格的随机访问心智错套到顺序读的文档上、为尾部问题造整套机制，已明确否决（2026-06）。
