...
> 本文是 spoor 阶段的产品路线图，基于 2026-06-11 的联网调研，取代 pith CLI 阶段的产品路线图。定位与工程规划见 [定位与工程规划](../planning/overview.md)。
...
| 原主张 | 自查结果 | 修正 |
| --- | --- | --- |
| A. 没有"单引擎多格式 WASM" | **被推翻**：@kreuzberg/wasm 已做到多格式+OCR | 收窄为："没有**塞得进免费边缘配额、适合浏览器插件**的多格式 WASM"（10.1 MB vs 目标 ≤3 MB gzip） |
| B. 没有"轻量纯 Rust 可嵌入 crate" | 部分成立：extractous 出局（混合架构+已停更）；LiteParse 依赖 PDFium/LibreOffice；kreuzberg 自称纯 Rust PDF 但 crate 体量是框架级 | 收窄为："没有**单一职责、依赖可控、MIT 许可证**的纯 Rust 文档引擎 crate" |
| C. Python 生态依赖臃肿 | 成立但在收窄：docling-slim 在瘦身；kreuzberg wheel 也免 torch 但仍有 27–34 MB | 卖点从"装得小"升级为"**wheel 个位数 MB + 离线确定性 + 防御契约**"（对标 tokenizers 2.5–10 MB 的工程水准） |
| D. 内建防御层（限制数据量/ZIP 炸弹/输出封顶）罕见 | **被推翻**（2026-06-11 源码核实）：kreuzberg 有完整且文档化更好的防御层（SECURITY.md 威胁模型 + ZipBombValidator + 提取超时 + XML 炸弹验证器），覆盖面比 spoor 现状只多不少 | 重写为**攻击面论述**："安全差异不在有没有防御，而在需要防御的面积"——spoor 是一个可以整体审计的小代码面（单一职责、依赖可控、无 C/C++ 组件），kreuzberg 的防御层是包在 90+ 格式解析器、PDFium、tree-sitter、143 个 LLM provider 外面的。另外向它学习：补一份同等水准的 SECURITY.md 威胁模型（已列入 P0） |

**修正后的定位语**：

spoor 要做文档解析领域的 jq 或 ripgrep——体积小、攻击面小、全栈纯 Rust（含 WASM 端，而竞品的浏览器 PDF 解析都依赖 PDFium / C++）、MIT 许可证、单一职责（文档 → LLM 友好文本）、内建 Agent 交互契约（Markdown-first + 表格收窄协议 + 稳定错误码）。
...
| PyO3 绑定（`pyspoor`） | maturin + abi3，wheel ≤ 10 MB，平台矩阵对齐 tokenizers；同一套测试用例跨入口通过 |
| 防御契约文档化 | 写一份 SECURITY.md 威胁模型（对标 kreuzberg 的水准：每条威胁 × 对应防御 × 默认值 × 可配置项）；限制单次解析的数据量 / ZIP 三重上限 / 输出封顶各自有测试清单；评估是否补"提取超时"（这是 kreuzberg 有而 spoor 目前没有的唯一防御项） |

### P1：WASM 与 Node（兑现差异化）

| 事项 | 验收标准 |
| --- | --- |
| `@harrisonwang/spoor-wasm` | **核心格式（DOCX/XLSX/CSV/HTML/MD）gzip ≤ 3 MB**，含 PDF 的完整版 ≤ 10 MB（按格式做编译特性开关，发布前实测）；浏览器拖拽 demo + CF Worker demo 跑通 |
| `@harrisonwang/spoor`（napi-rs） | optionalDependencies 平台子包模式（沿用 pith npm 包已验证的方案）；AnythingLLM/LobeChat 这类用 pdf-parse/pdfjs-dist 拼凑文档解析的项目可以一包替换 |
| 桌面集成样例 | Tauri 最小示例（直接 `cargo add spoor-core`） |

### P2：场景纵深（根据采用信号投入，不提前做）

- Electron / 本地 AI 客户端集成 PR 或适配指南（Jan 的文件上传仅支持 PDF、Cherry Studio / LobeChat 靠多个库拼凑解析，都是现成的切入点）。
- Lambda / S3 批量文档导入示例。
- 格式纵深（EPUB / HTML 渲染器统一等，继承自 pith 阶段未完成项）。
...
| REST API server / MCP server / Docker 化服务 | **不做** | 继承 pith 阶段的决策；kreuzberg 已占满该生态位，做就变成第二个它 |
...
- ~~kreuzberg 是否有等价的防御层~~ **已核实（2026-06-11，源码+SECURITY.md）：有，且文档化更好**。主张 D 已据此改写为攻击面论述，并将"补 SECURITY.md 威胁模型、评估提取超时"列入 P0。
