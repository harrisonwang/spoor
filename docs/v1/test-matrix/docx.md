...
| `docx/16_image_placeholders.docx` | `image_placeholders_follow_document_order_and_only_reference_safe_entries` | 图片按正文顺序输出安全 `spoor://docx/part/word/media/*` 占位符；过滤 fallback、HD Photo、外链和路径穿越；CLI `--extract` 原样输出单个安全资源 | Agent 可准确选择并提取需要交给 VLM 的图片，无需系统 unzip | passed | alt/caption、非 CLI 宿主提取接口 |
