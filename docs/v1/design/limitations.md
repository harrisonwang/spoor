...
| PDF | text layer、页顺序、`## Page N` 边界、`--pages` 页码区间；对清晰双栏版面基于字形几何重排阅读顺序并返回 `pdf_multi_column_reading_order` warning（保守、可回退）；混合文档的无文本页与明显可疑文本层返回带页码 warning | 不做 OCR；不恢复标题语义；多于两栏或复杂版面不保证阅读顺序（保守判定为单栏，回退原始顺序）；不去重页眉页脚、不修复断词；纯图片与加密 PDF 返回结构化错误 |
