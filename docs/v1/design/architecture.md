...
第二阶段采用 Cargo workspace，并优先发布 PyO3 绑定是合理路线，但不应仅因为"子进程看起来慢"就立即拆分。先用基准确认 CLI 启动和 IPC 已成为可见瓶颈，再把现有库接口层固化成稳定的类型化核心。
...
Rust package 可分别命名为 `spoor-core`、`spoor-cli`，CLI binary 命名为 `spoor`。Python distribution 名称在发布前单独确认；不要假设 `spoor` 在 PyPI 上可用。
...
PyO3 发布前，应把公共边界从 `anyhow::Result` 收敛到稳定的 `Result<T, SpoorError>`。内部仍可用 `anyhow` 补充上下文，但不能让它的动态错误文本成为跨语言契约。
...
Python distribution 建议使用私有 native module + 公共 Python 封装层：
...
- parsing 使用 `py.allow_threads(...)` 释放 GIL。
