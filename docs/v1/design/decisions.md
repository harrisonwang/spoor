...
这个 LLM-friendly 表示是按内容形态决定的，不按消费者类型决定。同一份 5000 行 XLSX 给 LLM 看 Markdown 表格是灾难（token 爆炸、丢表头）；给它 `{headers, preview_rows, row_count, range}` 则是常数 token + 一眼能看懂的形态。这就是为什么表格型默认走 JSON 而不是 Markdown，也是为什么文档型默认走 Markdown 而不是 JSON——把 prose 切 block 装进 JSON 信封，既昂贵又破坏顺序阅读的语义。
...
extractor 细节保持内部模块，不作为公共 API。第二阶段 workspace / `spoor-core` / PyO3
边界与迁移门槛见 [架构设计](architecture.md)。拆分前先把
公共 `anyhow::Result` 收敛为类型化结果/错误，并用 benchmark 验证 subprocess 开销是否真实构成瓶颈。
...
非表格格式使用 `-m json` 返回错误并提示使用 `-m md`。**不**为 DOCX/PDF/PPTX/IPYNB 设计通用 block-oriented JSON——把 prose 切 block 装进 JSON 信封既贵又破坏顺序阅读语义，是错误方向。
...
- 操作系统级精确 RSS / address-space 硬限制；当前数据量预算约束可控数据体积，不承诺覆盖第三方库所有短时分配。
...
- package name 保持 `spoor`（当前在 crates.io 可用）。
- binary name 保持 `spoor`。
- Homebrew/GitHub Release 是预编译二进制主路径。
- `cargo install --git https://github.com/harrisonwang/spoor` 是源码安装路径（更名后更新）。
...
- Rust library API：已有第一版接口层和 table JSON schema 类型；第二阶段先稳定类型化 core 契约，再保持行为不变地拆分并发布 PyO3 绑定。
...
- MCP server——shell + tool wrapper 已经够用；社区可以做 `spoor-mcp` thin wrapper。
- 通用 block-oriented JSON——文档型用 Markdown 已经是 LLM-friendly 表示。
- `spoor chunk` / 文档分块——文档是顺序读介质，截断是尾部小概率问题：缩小输入或 `--max-output-bytes` 重跑即可。为它造"清单 + 切片 + 收敛"整套机制，是把表格的随机访问心智错套到顺序读的文档上、为尾部问题造整套机制，已明确否决（2026-06）。
