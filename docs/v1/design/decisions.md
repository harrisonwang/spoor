...
- `--extract <uri>` 是格式无关的单资源二进制输出入口；统一形如 `spoor://<fmt>/<model>/<native-locator>`：`<fmt>` 段(pdf/docx/pptx/xlsx) 直接对应 core 的 `match format` 分派，`<model>` 段(`obj`/`part`)为同一格式预留新定位法。当前支持正文中实际输出的 `spoor://docx/part/word/media/*`、`spoor://pptx/part/ppt/media/*`、`spoor://pdf/obj/{id}/{gen}` 三类安全 URI，后续格式不新增专属 CLI flag。
...
- 内嵌媒体：`extract_media`；格式无关入口，当前支持安全 `spoor://docx/part/`、`spoor://pptx/part/` 与 `spoor://pdf/obj/` URI
...
- 内嵌栅格图片在正文中的安全 `spoor://docx/part/word/media/*` 占位符
...
- 字形几何 span 收集（内部 `pdf_layout` 中间模型）
- 清晰双栏版面的阅读顺序重排：保守判定，失败回退原始顺序，并返回
  `pdf_multi_column_reading_order` warning（带页码）；单栏与多于两栏的复杂版面
  保持原始顺序不变

待补：

- 断词修复
- 标题层级、页眉页脚分类、超过两栏的复杂版面

无文本层且无图片的 PDF（空白/纯矢量/损坏）返回非零退出码，并在 stderr 输出可解析的结构化错误：`code: "pdf_no_extractable_content"`、中文 reason/hint、`recoverable: true`（有图无文本层的扫描件不在此列，改为渲染页骨架+图片 marker 交给视觉模型）。受密码保护的 PDF 同理返回 `code: "encrypted_pdf"`、`recoverable: false`。结构化错误格式为 `{is_error, code, reason, hint, recoverable}`，消费者按 `code` 分支；`reason`/`hint` 是中文展示文本，不构成契约。全部稳定 code 见 README「结构化错误与 code」一节。
