---
name: spoor
description: 当 Agent 需要读取或检查本地文件、简单 URL，并把内容变成适合 LLM 使用的上下文时使用 spoor。适用于 PDF、DOCX、PPTX、EPUB、IPYNB、HTML/URL、Markdown、文本、代码、CSV/TSV、XLSX 等文件；在总结文档、提取需求、审阅资料、检查表格、基于本地附件回答问题时，优先用 spoor，而不是临时写 pandas/openpyxl、手搓解析器或直接 dump 原始文件。
tags:
  - docx
  - xlsx
  - pptx
...
用 `spoor` 先读文档和表格。它是本地 CLI 预处理工具，不是 OCR、在线办公 API、MCP server，也不是通用格式转换器。

## 怎么选命令

- 读支持的本地文件时，优先用 `spoor <path>`。
- 读简单的 HTML/Markdown 网页时，可以用 `spoor <url>`。
- 如果 `spoor` 不在 `PATH` 里，并且当前仓库就是 spoor 源码仓库，从仓库根目录运行 `cargo run --quiet -- <path>`。
- 文件类型不好判断时，加 `--format <format>` 明确指定。
- 默认输出模式交给 `spoor` 判断：文档型输出 Markdown，CSV/XLSX 输出 JSON。
- CSV/XLSX 只有在用户只是想快速瞄一眼小表时，才用 `-m md`。大表不要转 Markdown。
- PDF、DOCX、PPTX、EPUB、IPYNB、HTML、Markdown、文本这类文档不要用 `-m json`。
- spoor 没有 `--pages`、`--page`、`--ocr`、`--json`、`--output` 这类 flag；输出模式只有 `-m md|json`。PDF 页码已经以 `## Page N` 标题内嵌在 Markdown 输出里，要定位某一页，直接在输出里找对应标题。
...
1. 先运行 `spoor <file.csv>` 或 `spoor <file.xlsx>`。
...
- `spoor` 默认对整次命令 stdout 使用 256 KiB 总量上限，多文件和 glob 共享预算。看到截断信号时，不要假设后续内容不存在。
...
`spoor` 的结构化错误是 stderr 上的单行 JSON：`{"is_error":true,"code":"...","reason":"...","hint":"...","recoverable":...}`。**只按 `code` 分支**；`reason` 和 `hint` 是中文展示文本，可能改写，不要对它们做字符串匹配。

| code | 含义 | 正确的下一步 |
| --- | --- | --- |
| `image_only_pdf` | PDF 没有文本层 | 停止猜测内容，明确告诉用户需要外部 OCR；spoor 不做 OCR |
...
| `encrypted_pdf` | PDF 受密码保护 | 不可恢复，停止重试；请用户先解除密码。spoor 没有 `--password` 这类 flag |
...
| `parse_failed` | 其他解析器失败，已规范化并带 `stage` | 查看 `stage` 与 `hint`；确认文件完整后再决定是否重试 |

其他注意：

- 没有 JSON 信封的报错（stderr 纯文本）按 `reason` 原样如实上报，不要反复重试探测。
- 文档格式用 `-m json` 报错时，去掉 `-m json` 重新跑。
- 除非用户要求做取证排查，不要绕过 `spoor` 手动解压 Office/EPUB 文件。
- 如果任务是操作飞书、企微、钉钉等在线办公平台，用对应平台的 CLI/API skill；只有内容已经落成本地文件或简单 URL 时，才交给 `spoor` 读取。
...
spoor slides.pptx
spoor data.xlsx
spoor data.xlsx --sheet Revenue --columns month,region,revenue
spoor data.xlsx --sheet Revenue --rows 200:260
spoor data.csv --limit 20
spoor "docs/**/*.pdf"
```
