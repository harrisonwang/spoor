...
// Resolve one `spoor-docx://` placeholder emitted by `parse` into the raw
...
fn main() {
    ...
    // 演示按占位符提取内嵌图片字节：解析输出里的 spoor-docx:// 占位符可经此还原。
    let docx =
        include_bytes!("../../../crates/spoor-cli/tests/fixtures/docx/16_image_placeholders.docx");
    let image =
        extract_media_for_desktop(docx, "images.docx", "spoor-docx://word/media/image1.png")
            .expect("extract media");
    println!(
        "从 spoor-docx://word/media/image1.png 提取到 {} 字节",
        image.len()
    );
}
