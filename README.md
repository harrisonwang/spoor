...
```markdown
> [!WARNING]
> pith output truncated at the total limit of 262144 bytes. Content is incomplete; narrow the input or rerun with --max-output-bytes <n>.
```
...
{
  "is_error": true,
  "reason": "parse memory limit exceeded",
  "hint": "Parsing exceeded the available data-volume budget of 67108864 bytes during ... Narrow the input or rerun with --max-parse-bytes <n>.",
  "recoverable": true
}
```
...
```json
{"is_error":true,"reason":"image-only PDF","hint":"This PDF has no text layer. OCR is required, but pith does not perform OCR.","recoverable":true}
```
